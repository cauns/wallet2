	...
	public function addOrder($data) {
		...
		if (isset($data['totals'])) {
			foreach ($data['totals'] as $total) {
				//get total of order
				if($total['code']=='total'){
					$orderTotal =  -(float)$total['value'] ;
				}
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_total SET order_id = '" . (int)$order_id . "', code = '" . $this->db->escape($total['code']) . "', title = '" . $this->db->escape($total['title']) . "', `value` = '" . (float)$total['value'] . "', sort_order = '" . (int)$total['sort_order'] . "'");
			}
		}
		//sub money in wallet
		$description = "paid for order #".$order_id;
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer_payment
		SET description = '" . $description . "',
		   customer_id = '" . (int)$data['customer_id']  . "',
			 type = 2 ,
			 amount = '" . (float)$orderTotal . "',
			 date = NOW(),status = 1 ,del_id = 0  ");
		//check user enough money 
		$this->load->model('wallet/credit');
		$balance = $this->model_wallet_credit->getTotalBalance(); 
		if($balance >= $orderTotal){
			//update status order to processing
			$this->db->query("UPDATE " . DB_PREFIX . "order SET order_status_id = 2 where order_id = ".(int)$order_id );
			$remainBalance = $balance - $orderTotal;
			$message = "You have paid ". $orderTotal ." for order #".$order_id.".<br>Now your remain balance is ".$remainBalance ;
		}else{
			$message = "Your order #".$order_id. " cost ". $orderTotal .". But your remain balance is now ".$balance . "Please buy more credit to purchase for this order" ;
		...
		$from = $this->model_setting_setting->getSettingValue('config_email', $data['store_id']);
		if (!$from) {
		...
		$mail->setHtml($this->load->view('mail/current_balance', $data));
		$mail->setText($text);
		$mail->send();
		//end send mail
		return $order_id;
